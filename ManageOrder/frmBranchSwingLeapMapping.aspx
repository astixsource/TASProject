<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" CodeFile="frmBranchSwingLeapMapping.aspx.cs" Inherits="ManageOrder_frmBranchSwingLeapMapping" %>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#fraLeft').load(function () {
                ...
                $("#fraLeft").contents().find("#DvMenu").find("li[nid='25']").addClass("activemenu");
        ...
        function fnSaveFinalData() {
            ...
            var tbl = $("#tbldbrlist").find("input[type=radio]:checked");
            if (tbl.length == 0) {
                alert("Kindly Select atleast one branch for this Action!!!")
                return false;
            }
            ...
            var formatted_date = appendLeadingZeroes(current_datetime.getDate()) + "-" + months[current_datetime.getMonth()] + "-" + current_datetime.getFullYear()
                        ...
                        //}
                            ...
                            for (var i = 0; i < tbl.length; i++) {
                                var brnnodeid = tbl.eq(i).closest("tr").attr("brnnodeid");
                                var brnnodetype = tbl.eq(i).closest("tr").attr("brnnodetype");
                                var sitenodeid = tbl.eq(i).closest("tr").attr("sitenodeid");
                                var sitenodetype = tbl.eq(i).closest("tr").attr("sitenodetype");
                                var MappingTypeId = tbl.eq(i).closest("tr").find('input[type="radio"]:checked').val();
                                var TransferDate='';

                                if (tbl.eq(i).closest("tr").find('input[type="text"]').val() == "") {
                                    TransferDate=formatted_date;
                                }
                                else
                                {
                                    TransferDate = tbl.eq(i).closest("tr").find('input[type="text"]').val();
                                }
                                  
                                                            
                                arrDSENodeID.push({ BranchNodeId: brnnodeid, BranchNodeType: brnnodetype, SiteNodeId: sitenodeid, SiteNodeType: sitenodetype, MappingTypeId: MappingTypeId, TransferDate: TransferDate });
                            }
                        ...
                        var LoginId = $("#cphRight_hdnLoginId").val();
                        $("#dvFadeForProcessing").show();
                        PageMethods.fnSubmitSwingleapMapping(LoginId, arrDSENodeID, function (result) {
                            $("#dvFadeForProcessing").hide();
                            if (result.split("|")[0] == "2") {
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server" >
    ...
    <h4 id="h4header">>>Branch Swing/Leap Mapping</h4>
    ...
    <div id="divfixedHeader" style="margin-left: 0px; position:fixed;z-index:1"></div>
     <div id="divdrmmain"   style="margin-top:0px;border:1px solid #ccc;width:700px">
    </div>
        </div>
