<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" CodeFile="frmMarkAbsent.aspx.cs" Inherits="ManageOrder_frmMarkAbsent" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script>
        ...
        function fnDSEList() {
            ...
            PageMethods.fnDSEList(LoginId, BranchNodeId, BranchNodeType, function (result) {
                ...
                if (result.split("|")[0] == "2") {
                    ...
                    var totAbsenteeMarked = $("#tbldbrlist").find("tr[MarkAtt=1]").length;
                    var flgSubmitType = $("#tbldbrlist").is("[flgSubmitType]") ? $("#tbldbrlist").attr("flgSubmitType") : 0;
                    if (flgSubmitType == 2) {
                        $("#tbldbrlist").find("input[type=checkbox]:checked").attr("MarkAtt", "0");
                        $("#tdStatus").html("<b>Today Attendance Status : </b> Marked As No Absent");
                        //$("#divBTNS").find("a").hide();
                        ...
                        if (flgSubmitType == 1) {
                            ...
                            $("#tdStatus").html("<b>Attendance Status : </b> Yet Not Marked");
                            $("#anchorbtn2").find("div").html("No Absent");
                        }
                    }
        ...
        function fnChangeSectorRoute(sender) {
            ...
            $("#dvDialog").dialog({
                ...
                buttons: {
                    "Change": function () {
                        var cSectorId = $("#ddlSector option:selected").val();
                        var cSector = $("#ddlSector option:selected").text();
                        var cRouteId = $("#ddlRoute option:selected").val();
                        var cRoute = $("#ddlRoute option:selected").text();
                        $(sender).closest("tr").attr("SectorId", cSectorId);
                        $(sender).closest("tr").attr("RouteNodeId", cRouteId.split("_")[0]);
                        $(sender).closest("tr").attr("RouteNodeType", cRouteId.split("_")[1]);
                        var tds = $(sender).closest("tr").find("td");
                        tds.eq(3)[0].innerHTML = cSector;
                        tds.eq(4)[0].innerHTML = cRoute;
                        $(sender).closest("tr").removeClass("clsHighlightrowsChangeRoute");
                        if (parseInt(OldSectorId) != parseInt(cSectorId) || parseInt(OldRouteNodeId) != parseInt(cRouteId.split("_")[0])) {
                            $(sender).closest("tr").addClass("clsHighlightrowsChangeRoute");
                        }
                        $(this).dialog('close');
                    },
                    "Cancel": function () {
                ...
                open: function () {
                    var arr1 = jQuery.grep(arrRouteData[0], function (element, index) {
                        return (parseInt(element.DSENodeId) == parseInt(DSENodeId) && parseInt(element.DSENodeType) == parseInt(DSENodeType));
                    });

                    var arr2 = jQuery.grep(arrSectorData[0], function (element, index) {
                        return (parseInt(element.DSENodeId) == parseInt(DSENodeId) && parseInt(element.DSENodeType) == parseInt(DSENodeType));
                    });
                    ...
                    for (var i in arr1) {
                        var strSelected = "";
                        if (parseInt(RouteNodeId) == parseInt(arr1[i]["NodeID"])) {
                            strSelected = "selected='selected'";
                        }
                        strRoute += "<option " + strSelected + " value='" + (arr1[i]["NodeID"] + "_" + arr1[i]["NodeType"]) + "'>" + arr1[i]["Descr"] + "</option>";
                    }

                    var strSector = "";
                    for (var i in arr2) {
                        var strSelected = "";
                        if (parseInt(SectorId) == parseInt(arr2[i]["SectorId"])) {
                            strSelected = "selected='selected'";
                        }
                        strSector += "<option " + strSelected + "  value='" + arr2[i]["SectorId"] + "'>" + arr2[i]["SectorCode"] + "</option>";
                    }

                    var strHTML = ("<table style='width:100%'>");
                    var strStyle = "padding:4px";
                    strHTML += ("<tr>");
                    strHTML += ("<td style='" + strStyle + "'>Sector</td><td style='" + strStyle + "'>:</td><td style='" + strStyle + "'><select id='ddlSector'>" + strSector + "</select></td>");
                    strHTML += ("<tr>");

                    strHTML += ("<tr>");
                    strHTML += ("<td style='" + strStyle + "'>Route</td><td style='" + strStyle + "'>:</td><td style='" + strStyle + "'><select  id='ddlRoute'>" + strRoute + "</select></td>");
                    strHTML += ("<tr>");
                    strHTML += "</table>";
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server">
    ...
    <h4 id="h4header">>>Mark DSE Absent</h4>
    ...
    <div class="form-inline" id="tblhead" style="margin-top: 10px;">
        ...
        <div class="input-group">
            <div class="pull-left" style="padding-right:5px" id="tdStatus">
               <b>Attendance Status</b>: Not Marked
            </div>
            <div class="pull-left" >
                <b>Absent For Today</b>: <%=DateTime.Now.ToString("dd-MMM-yy") %>
            </div>
