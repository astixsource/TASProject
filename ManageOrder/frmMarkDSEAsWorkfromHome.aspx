<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" CodeFile="frmMarkDSEAsWorkfromHome.aspx.cs" Inherits="frmMarkDSEAsWorkfromHome" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script>
        ...
        function fnMarkDSEWorkFromHome(Sender,flg) {
            var IsSubmitType = flg;
            if (IsSubmitType == 1) {
                if ($("#tbldbrlist").find("input[type=checkbox]:checked").length > 0) {
                    ...
                    $("#dvDialog")[0].innerHTML = "Are you sure to mark as work from home for these selected DSE?"
                }
            } else {
                IsSubmitType = 2;
                $("#dvDialog")[0].innerHTML = "Are you sure to mark as Normal Field Work for this DSE?"
            }
            ...
            $("#dvDialog").dialog({
                ...
                buttons: {
                    "Yes": function () {
                        ...
                        if (IsSubmitType == 2) {
                            var DSECallScheduleID = $(Sender).closest("tr").attr("DSECallScheduleID");
                            var DSENodeId = $(Sender).closest("tr").attr("DSENodeId");
                            var DSENodeType = $(Sender).closest("tr").attr("DSENodeType");
                            arrDSENodeID.push({ DSECallScheduleID: DSECallScheduleID, DSENodeID: DSENodeId, DSENodeType: DSENodeType, flgWFH: flg });
                        } else {
                            for (var i = 0; i < tbl.length; i++) {
                                var DSECallScheduleID = 0;// tbl.eq(i).closest("tr").attr("DSECallScheduleID");
                        ...
                        var LoginId = $("#cphRight_hdnLoginId").val();
                        $("#dvFadeForProcessing").show();
                        PageMethods.fnSubmitDSEAttendance(LoginId, arrDSENodeID, BranchNodeId, BranchNodeType, function (result) {
                            $("#dvFadeForProcessing").hide();
                            if (result.split("|")[0] == "2") {
                                ...
                                if (IsSubmitType == 1) {
                                    ...
                                    $("#tbldbrlist").find("input[type=checkbox]:checked").closest("td").next().html("<a href='###' style='color:blue;text-decoration:underline' onclick='fnMarkDSEWorkFromHome(this,2)'>Normal Field Work</a>");
                                } else {
        ...
        function fnDownloadCredential() {
            var BranchVal = $("#cphRight_ddlBranch").val();
            var BranchNodeId = BranchVal.split("-")[0];
            var BranchNodeType = BranchVal.split("-")[1];
            var BranchCode = $("#cphRight_ddlBranch option:selected").val();
            var LoginId = $("#cphRight_hdnLoginId").val();
            window.open("../frmDownloadExcel.aspx?flg=10&&BranchCode=" + BranchCode + "&&BranchNodeId=" + BranchNodeId + "&BranchNodeType=" + BranchNodeType);
        }
    </script>
</asp:Content>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server">
    ...
    <h4 id="h4header">>>Mark DSE As Work From Home</h4>
    ...
    <div class="form-inline" id="tblhead" style="margin-top: 10px;">
        ...
        <div class="input-group">
         ...
         <div class="form-group">
            <input type="button"  value="Download DSE Credential" class="btn btn-primary" onclick="fnDownloadCredential()"  />
        </div>
    </div>
    ...
    <div class="blockButtons" id="divBTNS" style="display: none; bottom: 0; position: fixed; padding-top: 1px; background-color: #cfcfcf; overflow-x: auto;">
        <table style="min-width: 405px;">
            <tr>
                <td>
                    <a href="###" class="icon-bnt" onclick="fnMarkDSEWorkFromHome(this,1)" id="anchorbtn2"><span class="PostOrder"></span>
                        <div style="font-size: 7.8pt; width: 170px; line-height: 28px">Mark Work From Home</div>
                    </a>
                </td>
