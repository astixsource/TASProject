<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" CodeFile="frmMarkTeleCallerAbsent.aspx.cs" Inherits="ManageOrder_frmMarkTeleCallerAbsent" %>
    ...
    *top: 0.1em;
    ...
    <script>
        ...
        function fnTelecallerList() {
            ...
            PageMethods.fnTelecallerList(LoginId, TSVNodeId, TSVNodeType, function (result) {
        ...
        function fnSaveFinalData() {
            var tbl = $("#tbldbrlist").find("input[MarkAtt=0][type=checkbox]:checked");
            if (tbl.length == 0) {
                alert("Kindly Select Telecaller to mark absent first!!!")
                return false;
            }
            $("#dvDialog")[0].innerHTML = "Are you sure to mark as absent for these selected Telecaller?"
            $("#dvDialog").dialog({
                modal: true,
                title: "Confirmation:",
                buttons: {
                    "Yes": function () {
                        ...
                        for (var i = 0; i < tbl.length; i++) {
                            flgAbsent = 1;
                            var TeleCallerId = tbl.eq(i).closest("tr").attr("TeleCallerId");
                            var NodeType = tbl.eq(i).closest("tr").attr("TeleNodeType");
                            arrDSENodeID.push({ DSENodeID: TeleCallerId, DSENodeType: NodeType, RouteNodeId: 0, RouteNodeType: 0, SectorId: 0, flgAbsent: flgAbsent, flgChangeRouteSector: 0 });
                        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server" >
    ...
    <h4 id="h4header">>>Mark Telecaller Absent</h4>
    <div style="margin-top:10px;width:100%">
      ...
      <%--<input type="text" class="form-control" placeholder="Search Telecaller" style="width:100%;margin-bottom:3px;border: 1px solid #bbbbbb;"  id="txtFindDbr" />--%>
                    </td>
                <td style="vertical-align:middle;text-align:center">
                    <div style="display:inline-block;font-size:10pt;font-weight:bold;line-height:2;">
                        Absent For Today : <%=DateTime.Now.ToString("dd-MMM-yyyy") %>
                    </div>
            ...
            <tr>
                ...
                <td>
                    <table>
                        ...
                        <td style="width:30px;background-color:#ffd5d5">
