<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" CodeFile="frmBranchInventryMapping.aspx.cs" Inherits="frmBranchInventryMapping" %>
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#fraLeft').load(function () {
                ...
                $("#fraLeft").contents().find("#DvMenu").find("li[nid='51']").addClass("activemenu");
        ...
        function fnSaveFinalData() {
            ...
            var $checked = $("#tbldbrlist").find("input[type='checkbox'][flg=1]");
            //if (tbl.length == 0) {
            ...
            for (var i = 0; i < $checked.length; i++) {
                ...
                if ($checked.eq(i).is(":checked")) {
                    ...
                    if ($checked.eq(i).closest("td").next().find("input[type='checkbox']:checked").length==0) {
                        $("#dvFadeForProcessing").hide();
                        alert("Select Inventory From First!");
                        $checked.eq(i).closest("td").next().find("input[type='checkbox']").eq(0).focus();
                        return false;
                    }
                    if ($checked.eq(i).closest("td").next().find("input[type='checkbox']").eq(0).is(":checked") == true) {
                        arrDSENodeID.push({ BranchNodeId: brnnodeid, BranchNodeType: brnnodetype, RefNodeId: brnnodeid, RefNodeType: brnnodetype, flgUseSystemPrice: flgUseSystemPrice });
                    }

                    if ($checked.eq(i).closest("td").next().find("input[type='checkbox']").eq(1).is(":checked") == true) {
                        if ($checked.eq(i).closest("td").next().find("select").val() == "0") {
                            $("#dvFadeForProcessing").hide();
                            alert("Select Hub First!");
                            $checked.eq(i).closest("td").next().find("select").focus();
                            return false;
                        }
                        RefNodeId = $checked.eq(i).closest("td").next().find("select").val();
                        RefNodeType = $checked.eq(i).closest("td").next().find("select option:selected").attr("brnnodetype");
                        arrDSENodeID.push({ BranchNodeId: brnnodeid, BranchNodeType: brnnodetype, RefNodeId: RefNodeId, RefNodeType: RefNodeType, flgUseSystemPrice: flgUseSystemPrice });
                    }
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server">
    ...
    <div style="margin-left: 0px; padding-bottom: 5px; position: fixed; z-index: 1; width: 100%; background-color: #ffffff" id="divHeadercont">
        <h4 id="h4header">>>Branch Inventory Mapping</h4>
