...
public partial class ManageOrder_frmRouteList_Telecaller : System.Web.UI.Page
{
    ...
    public static string fnStoreList(int LoginId, int MenuId, int SaleNodeType)
    {
        ...
        try
        {
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                ...
                SkipColumn[6] = "Store Details^Store Code";
                SkipColumn[6] = "Store Details^Store Code";
                SkipColumn[7] = "On Route^flgOnRoute";
                SkipColumn[8] = "DBRName";
                SkipColumn[9] = "flgRecording";

                //StartDate = DateTime.Parse(Ds.Tables[0].Rows[0]["StartDate"].ToString()).ToString("dd MMM");
                str.Append("<table id='tbldbrlist' style='width:100%;border-top:1px solid #bbbbbb;border-right:1px solid #bbbbbb;font-size:8pt' ><thead><tr>");
                ...
                for (int k = 0; k < Collength.Length; k++)
                {
                    ...
                    if (cntt == 0)
                    {
                        str.Append("<th style=';padding:2px;background-color:#26a6e7;color:#ffffff;vertical-align:middle;text-align:center;width:3%;border-left:1px solid #bbbbbb;border-bottom:1px solid #bbbbbb' rowspan='2' >SrNo</th>");
                    }
                    cntt++;
...
//string sdatass = Convert.ToString(Ds.Tables[0].Rows[i]["Store Details^Contact No"]).Length > 7 ? "<a href ='###'onclick ='fnEditOrder(this)' style = 'color:blue;text-decoration:underline;' title='Click to book order' >" + Ds.Tables[0].Rows[i][j].ToString() + " - " + Ds.Tables[0].Rows[i]["Store Details^Store Code"].ToString() + "</a>" : Ds.Tables[0].Rows[i][j].ToString() + " - " + Ds.Tables[0].Rows[i]["Store Details^Store Code"].ToString();
                        ...
                        if(sColumnName == "Store Details^Contact No")
                        {
                            ...
                            if (strContact.Length > 7)
                            {
                                ...
                                {
                                        string strLink = "";
                                        strLink += strContact.Split('/')[0].Trim().Length > 7 && flgOrderStatus < 3 ? "<a href ='###' flg='1' onclick ='fnEditOrder(this)' style ='color:blue !important;text-decoration:underline !important;' title='Click to make call' >" + strContact.Split('/')[0].Trim() + "</a>" : Convert.ToString(strContact.Split('/')[0]).Trim();
                                        strLink += strContact.Split('/')[1].Trim().Length > 7 && flgOrderStatus < 3 ? "<br/><a href ='###' flg='1' onclick ='fnEditOrder(this)' style ='color:blue !important;text-decoration:underline !important;' title='Click to make call' >" + strContact.Split('/')[1].Trim() + "</a>" : "<br/>" + Convert.ToString(strContact.Split('/')[1]).Trim();
                                        sdata = strLink;
                                }
                                else
                                {
                                        sdata = strContact.Trim().Length > 7 && flgOrderStatus < 3 ? "<a href ='###' flg='1' onclick ='fnEditOrder(this)' style ='color:blue !important;text-decoration:underline !important;' title='Click to make call' >" + strContact.Trim() + "</a>" : strContact.Trim();
                                }
                            }
                            else
                            {
            ...
            //{
                ...
                strTotstring.Append("</table>");
                stresponse += "|" + strTotstring.ToString()+"|"+ JsonConvert.SerializeObject(Ds.Tables[2], Formatting.Indented, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore }) + "|" + JsonConvert.SerializeObject(Ds.Tables[3], Formatting.Indented, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore }); 
            //}
        }
        catch (Exception ex)
        {
    ...
    public static string fnSentNotification(string MobileNo,string TokenNo, string StoreId, string TeleCallingId, string flgRecording)
    {
        ...
        try {
            ...
            string apiURL = ConfigurationManager.AppSettings["TeleCallingDataAPI_URL"];
            string createSaleReturnUrl = apiURL + "/Default/SendNotification_TAS?PersonId=0&Token=" + TokenNo + "&Msg=" + MobileNo + "&flg=0&StoreId=" + StoreId + "&TeleCallingId=" + TeleCallingId + "&flgRecording=" + flgRecording;
