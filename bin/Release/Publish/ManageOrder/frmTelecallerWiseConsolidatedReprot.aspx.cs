...
public partial class frmTelecallerWiseConsolidatedReprot : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["LoginID"] == null)
        {
            ...
            if (!IsPostBack)
            {
                ...
                hdnMenuId.Value = Request.QueryString["id"] == null ? "12" : Request.QueryString["id"].ToString();
                string flg = Request.QueryString["flg"] == null ? "1" : Request.QueryString["flg"].ToString();
                hdnflg.Value = flg;
                if (flg == "2")
                {
                    lblHeader.InnerHtml = ">>Daily SUBD Consolidated Report";
                    lblType.InnerHtml = "SUBD List";
                }
                else
                {
                    lblHeader.InnerHtml = ">>Daily Branch Consolidated Report";
    ...
    private static string multilvlPopuptbl(DataTable dt, int col_ind, int row_ind)
    {
        ...
        if (row_ind == 1)
        {
            ...
            else if (str1 == "Calling Status")
            {
                if (str2 == "Reason")
                {
                    flgcolor = "style='background-color:#00ca65;color:#ffffff;text-align:center;border-left:1px solid #bbbbbb;border-bottom:1px solid #bbbbbb;padding:2px;width:10%'";
                }
                else
                {
        ...
        return " <th " + flgcolor + " class='clspopuptblhead_" + row_ind + "_" + col_ind + " cls" + dt.Columns[col_ind].ColumnName.ToString().Split('^')[0].Split(' ')[0] + "' colspan='" + (cntr- colInt) + "'> " + str + " </th>|" + cntr;
    }


    //Get Scheme And Product Detail Bases on Store
    [System.Web.Services.WebMethod()]
    public static string fnBranchOrderStatusList(int LoginId, int BranchNodeId, int BranchNodeType, int SiteNodeid,int SiteNodeType,string RouteDate,string flg)
    {
        ...
        try
        {
            string storedProcName =flg=="1"? "spDownloadTeleCallerOrderDetail": "spDownloadTeleCallerSUBDOrderDetail";
            List<SqlParameter> sp = new List<SqlParameter>()
                    {
                   new SqlParameter("@RouteDate", RouteDate),
            ...
            if (Ds.Tables[0].Rows.Count > 0)
            {
                    ...
                    for (int j = 0; j < Ds.Tables[0].Columns.Count; j++)
                    {
                        if (!SkipColumn.Contains(Ds.Tables[0].Columns[j].ColumnName.ToString().Trim()))
                        {
                            ...
                            if (ColSpliter[k] != "")
                            {
                                ...
                                {
                                    string sColumnName = Ds.Tables[0].Columns[j].ColumnName.ToString().Split('^')[0];
                                    if (sColumnName == "Last Call Date" || sColumnName == "Language" || sColumnName == "TeleUser")
                                    {
                                        ss = "style='background-color:#26a6e7;color:#ffffff;vertical-align:middle;text-align:center;border-left:1px solid #bbbbbb;border-bottom:1px solid #bbbbbb;padding:2px;width:5%'";
                                    }
                ...
                for (int i = 0; i < Ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    str.Append("<tr " + (flgOrderStatus > 2 ? "title='Order has been downloaded'" : "") + "  >");
                    ...
                    for (int j = 0; j < Ds.Tables[0].Columns.Count; j++)
                    {
                        ...
                        {
                            sdata = Ds.Tables[0].Rows[i][j].ToString()+" ("+ Ds.Tables[0].Rows[i]["Store Details^Store Code"].ToString()+")";
