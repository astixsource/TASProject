<%@ Page Title="" Language="C#" MasterPageFile="~/site.master" AutoEventWireup="true" EnableEventValidation="false" CodeFile="frmDRCPUpload.aspx.cs" Inherits="_Default" %>
    ...
    <script>
            ...
            $('input[type=file]').bind("change", function () {
                ...
                if ($(this).val() != "") {
                    ...
                    if (selectedFile.split("_").length != 3) {
                        $(this).val('');
                        alert("Invalid File naming convention,kindly upload correct file format only like \"DRCP_BranchCode_yyyyMMdd\"");
                        return false;
                    }
                    if (selectedFile.split("_")[0].toLowerCase() != "drcp") {
                        $(this).val('');
                        alert("Invalid File naming convention,kindly upload correct file format only like \"DRCP_BranchCode_yyyyMMdd\"");
                        return false;
                    }
                    ...
                    if ($(this).val() != "") {
                        ...
                        $(this).closest("tr").addClass("clsFileHighlighted");
                    }
                }
            });
        });
        ...
        function fnUploadFiles() {
            ...
            for (var i = 0; i < files.length ; i++) {
                ...
                var branchcode = $("#" + files[i].id).closest("tr").find("td").eq(1).html().split("[")[1].replace("]", "");
                var branchnodeid = $("#" + files[i].id).closest("tr").attr("brnnodeid");
                var branchnodetype = $("#" + files[i].id).closest("tr").attr("brnnodetype");
                var RptMonthYear = $("#" + files[i].id).closest("tr").find("select").eq(0).val();
                var LoginId = $("#cphRight_hdnLoginId").val();
                var formdata = new FormData();
                formdata.append("file1", file[0]);
                formdata.append("branchcode", branchcode);
                formdata.append("branchnodeid", branchnodeid);
                formdata.append("branchnodetype", branchnodetype);
                formdata.append("RptMonthYear", RptMonthYear);
                formdata.append("LoginId", LoginId);
                postFile(formdata, files[i].id);
            }
        }
        function postFile(data, id) {
            // Make the Ajax call
            $.ajax({
                url: "FileDRCPUploadHandler.ashx",
                type: 'POST',
                data: data,
                contentType: false,
                processData: false,
        ...
        function fnSuccess(response, id) {
            if (response.split("^")[0] == "0" || response.split("^")[0] == "2") {
                $("#" + id).closest("td").next().html(response.split("^")[1]);
                $("#" + id).val("");
                $("#" + id).attr("flgUpload", "0");
                var d = new Date('<%=DateTime.Now%>');
                $("#" + id).closest("tr").find("td").eq(3).html(d.localeFormat("dd-MMM-yy hh:mm tt"));
                $("#" + id).closest("tr").find("td").eq(5).html("<a href='###' style='font-size:9pt;color:blue;text-decoration:underline' onclick='fnDownloadRouteCalendar(this,1)'>Download<a>");
                $("#" + id).closest("tr").removeClass("clsFileHighlighted");
            } else {
                $("#" + id).attr("flgUpload", "0");
                $("#" + id).val("");
                $("#" + id).closest("td").next().html(response.split("^")[1]);
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphRight" runat="Server">
    ...
    <div style="margin: 7px 80px; border: 1px solid #23aed8;" id="divMain">
        <div id="divHeaderOne" style="padding-bottom:12px;background:#ffffff;margin-left: 0px; position: fixed; z-index: 1; width: 83%;">
            ...
            <div style="padding:5px;">
                <table style="width: 100%">
                    <tr>
                        <td style="width: 15%">File Naming Convention</td>
                        <td>:</td>
                        <td>DRCP_BranchCode_yyyyMMdd.xls / .xlsx / .xlsb</td>
                    </tr>
                    <tr>
                        <td>Sample File</td>
                        <td>:</td>
                        <td><a href="SampleFile/DRCP/Swing/DRCP_BranchCode_yyyyMMdd.xlsx" target="_blank" style="color: blue; text-decoration: underline">DRCP_BranchCode_yyyyMMdd.xlsx</a></td>
                    </tr>
                </table>
            </div>
        </div>
    ...
    <div id="divbtns" style="bottom: 0px; position: fixed; margin: 0px 80px; width: 100%; background-color: lightgray; padding: 3px" class="text-center">
        <input type="button" value="Upload" onclick="fnUploadFiles()" class="btn btn-primary" />
    </div>
