...
$(document).ready(function () {
    ...
    $('#ddlGroupBy').change(function () {
        ...
        $("#cphRight_TViewChannel").css("display", "none");

        fnGetUnProductSelection();
        fnGetUnSalesSelection();
        fnGetUnChannelSelection();
...
function GetNodeTypeForGroup(ele) {
    ...
    if (lvl != 0) {
        ...
        $("div[activeflag='" + 12 + "']").each(function () {
            $(this).children("input[type='checkbox']").css("display", "inline");
            $(this).children("input[type='checkbox']").parent("div").attr("flg", "13");
        });
    }
    if (MainLevel == 1) {
        filterid = 6;
        ...
        $("#cphRight_TreeviewBusinessUnit").css("display", "none");
        $("#cphRight_TreeviewSales").css("display", "none");
        $("#cphRight_TViewChannel").css("display", "block");
    }
    if (lvl != 0) {
        $("#cphRight_hdnGeographyfilter").val("");
        ...
        if (MainLevel == 1) {
            ...
            // bind click event on checkbox for reset
            fntreechkboxdisableForPopup("cphRight_TViewChannel", 12, 14, 13);
        }
        else {
            $("div[activeflag='" + filterid + "']").each(function () {
                $(this).closest("table").css("display", "none");
    ...
    if (selType == 1) {
        ...
        if (MainLevel == 6) {
            ...
            for (var j = 0; j < childEle1.length; j++) {
                $(childEle1[j]).closest("table").find("img").css("display", "none");
                $(childEle1[j]).children("input[type='checkbox']").prop("checked", true);
                $(childEle1[j]).children("input[type='checkbox']").parent("div").attr("flg", "14");
            }
        }
    }
    if (selType == 2) {
...
function fnHideFilterOnFilterTab() {
    ...
    if ($("#ddlGroupBy").val() == 6 && $("#cphRight_hdnChannel").val() != "") {
        $("#dv_tab_8").hide();
        $("#dv_tab_1").show();
        $("#dv_tab_3").show();
        $("#dv_tab_4").show();
    }
    if (($("#ddlGroupBy").val() != 6 || $("#ddlGroupBy").val() == 6) && $("#cphRight_hdnChannel").val() == "") {
        $("#dv_tab_8").show();
    }
...
function CheckingPopupAdditionalFilter() {
    ...
    else if ($("#hdnCallFrom").val() == "0" && $("#ddlGroupBy").val() == "6") {
        if ($("#ddlSubGroupOption").val() == null || $("#ddlSubGroupOption").val() == "" || $("#ddlSubGroupOption").val() == "0") {
            btnServiceByOK_onclick();
            btngeographyByOK_onclick();
            btnGetDistributerTree_onClick();
...
function fnGetChannelSelection() {
    $("#cphRight_hdnChannel").val('');
    $("div[activeflag='12']").each(function () {
        if ($(this).attr("flg") == "14") {
            var val = ((this.id).toString().substring(4)).split('_');
...
function fnGetUnChannelSelection() {
    var childEle1 = $("div[activeflag='12']");
    for (var j = 0; j < childEle1.length; j++) {
        if ($(childEle1[j]).children("input[type='checkbox']").parent("div").attr("flg") == 14) {
            $(childEle1[j]).children("input[type='checkbox']").prop("checked", false);
            $(childEle1[j]).children("input[type='checkbox']").parent("div").attr("flg", "13")
            $("#cphRight_hdnChannel").val('');
        }
    }
}
...
function FnSetParametersForUserChartDemo_Success(result) {
    ...
    for (var Dr1 in dtItems[0].Table) {
        ...
        if ($("#cphRight_hdnCallFrom").val() != 1) {
            ...
            $(Tc).find(".icons").append("<div style='display: inline-block; width:auto; border-left: 1px solid #e9e9e9;padding:4px 6px;'><div style='position:relative;'><img id='img" + Seq + "' src='images/flicon.png' alt='View Filter' width='16px' height='16px' onmouseover='DisplayfilterPopup(this,event);' onmouseout='HidefilterPopup(this)'><div class='filter' style='display:none; position:absolute; right: 0px; top:20px'>" + fnDisplayFilter(strViewBy, strGeoGraphy, strServiceLine, strTime, strDBSales, strChannel) + "</div></div></div>");
...
function DisplayfilterPopup(ele, event) {
    $(ele).siblings('div').css("display", "block");
}
function HidefilterPopup(ele) {
    $(ele).siblings('div').hide();
}
function getBrowser() {
