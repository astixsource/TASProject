...
var DiscountIndx = 7;
var BestRateIndx = 8;
var ValueBeforeTaxIndx = 9;
var ValueAfterTaxIndx = 10;
...
function fnShowLastFiveOrders(arrData) {
    flgTaskEnable = 0;
    var strHML = "";
    var arr = ["PrdNodeId", "PrdNodeType", "PcsInBox", "Category", "CategoryId", "CategoryNetValue", "SBFNetValue", "OrderType", "SBDGroup", "CatNodeID", "SBDGroupId", "flgBaseProduct", "SBDPrdCnt", "InvLevelDisc", "flgInitiative", "flgInactive", "flgFB", "Brand", "brandNetValue", "FBName", "FBID", "IsFBProductShow", "flgSBD", "SBDStrCnt", "Reco_Strategy", "SeqNo", "flgSmartBasket", "flgDefaultSmartBasket", "CategorySB"];
    var currDate = $("#cphRight_hdnCurrentDate").val();
    var d = new Date(currDate);
                ...
                $.each(Table[i], function (key, value) {
                    if ($.inArray(key, arr) == -1) {
                        ...
                        if (key == "Product Name") {
                            ...
                            flgSmartBasket
                            if (flgDefaultSmartBasket == 1) {
                                strPromoimgage = " <div class='clsDivSBIcon' style='border:2px solid #00B0F0;'>SB</div>";
                            }
                            else if (flgSmartBasket == 2 && flgDefaultSmartBasket == 0) {
                                strPromoimgage = " <div class='clsDivSBIcon' style='border:2px solid #FE4225'>SB</div>";
                            }
...
function orderBookingTotalCalc(Sender, ProductID, alrtBenSubBucketType, listfreeProductQty, schemAllString) {
    ...
    if (trPrdItemsMain.length > 0) {
        for (var i = 0; i < trPrdItemsMain.length; i++) {
            ...
            trPrdItemsMain[i].cells[DiscountIndx].innerHTML = "&#8377; " + parseFloat(StandardRateBeforeTax).toFixed(2);
...
function fnCalculateData(cntrl) {
    ...
    if (skuID != undefined) {
        ...
        $(cntrl).closest("tr").find("td[iden='disc']").html("&#8377; " + parseFloat(standardratebeforetax).toFixed(2));
