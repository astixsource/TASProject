...
function fnValidateIMEINos() {
    var _validMessage = "";
    var imeiNo1 = $("#" + _mainPanelName + "txtPdaSerielMstr").val().trim();
    var imeiNo2 = $("#" + _mainPanelName + "txtPdaIEMIMaster2").val().trim();

    if (imeiNo1 != "" || imeiNo2 != "") {
        $("#" + _mainPanelName + "lstBoxPdaMaster option").each(function () {
            ...
            if (val.split("^")[0] != parseInt($("#hiddenNodeID").val())) {
                if (imeiNo1 != "") {
                    if (text.split(',').length > 0) {
                        if (text.split(',')[0] == imeiNo1) {
                            _validMessage = "IMEI Number 1 is already present.";
                        }
                    }
                    if (text.split(',').length > 1) {
                        if (text.split(',')[1] == imeiNo1) {
                            _validMessage = "IMEI Number 1 is already present.";
                        }
                    }
                }

                if (imeiNo2 != "" && _validMessage == "") {
                    if (text.split(',').length > 0) {
                        if (text.split(',')[0] == imeiNo2) {
                            _validMessage = "IMEI Number 2 is already present.";
                        }
                    }
                    if (text.split(',').length > 1) {
                        if (text.split(',')[1] == imeiNo2) {
                            _validMessage = "IMEI Number 2 is already present.";
                        }
                    }
                }
...
function fnValidateControls() {
    ...
    // Validate PDA Master
    if (pdaType == 1) {
        if ($("#" + _mainPanelName + "txtPdaSerielMstr").val().trim() == "")
            _validMessage = "IMEI Number 1 is Required";
        else if ($("#" + _mainPanelName + "txtPdaSerielMstr").val().trim() != "" || $("#" + _mainPanelName + "txtPdaIEMIMaster2").val().trim() != "")
            _validMessage = fnValidateIMEINos();
        else if ($("#" + _mainPanelName + "txtPdaPurchseDate").val().trim() == "")
            _validMessage = "Purchase Date is Required";
        else if ($("#" + _mainPanelName + "txtPdaPurchseFrom").val().trim() == "")
            _validMessage = "Purchase From is Required";
    }
    // Validate SIM Master
    if (pdaType == 2) {
        if ($("#" + _mainPanelName + "txtPdaSimSeriel").val() == "")
            _validMessage = "SIM Number is Required";
            ...
            _validMessage = "Cell Number is Required";
    }
    // Validate Service Provider
    if (pdaType == 3) {
        if ($("#" + _mainPanelName + "txtServiceProviderName").val() == "")
            _validMessage = "Service Provider name is Required";
    }
    ...
    if (pdaType == 4) {
        ...
        //            _validMessage = "Please Make SIM Selection";
            ...
            _validMessage = "Allcation Date is Required";
