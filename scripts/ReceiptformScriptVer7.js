...
function fnAddDynamicRowInProductItemsTable(flg, tableName, col, row, numericColIndex, dateColIndex, isfileUpload, fileUploadColIndex,selectIfscColIndex, selectBankColIndex, selectBankBranchColIndex,selectModeColIndex,selectRefColIndex, result) {
    //debugger;
    result = result || "";
    var jRecord = "";
    if (result != "")
    ...
    var IfsccolIndex = selectIfscColIndex.split("^");
    ...
    if (flg == 0) {
        ...
        for (var i = 0; i < row; i++) {
            ...
            var flgbbranch = 0;
            for (var j = 0; j < col; j++) {
                var value = ""; var bbranch = ""; var bankName = ""; var flgIfsccode = 0; var flgbbranch = 0; var flgbank = 0; var flgTrnRefNo = 0; var flgDate = 0; var flgAmount = 0; var flgRemarks = 0; var flgInsMode = 0;
                if (jRecord != "") {
                    ...
                    if (tblId == "tbldd" || tblId == "tblCheque") {
                        if (j == 0) {
                            ...
                            flgIfsccode = 1;
                            value = jRecord[i].IFSCCode;
                        }
                        else if (j == 4) {
                            flgbank = 1;
                            value = jRecord[i].BankId;
                            bankName = jRecord[i].BankName;
                        }
                        else if (j == 5) {
                            value = jRecord[i].BankBranchId;
                            bbranch = jRecord[i].BankBranchName;
                            flgbbranch = 1;
                        }
                        else if (j == 6){
                ...
                if ($.inArray("" + j + "", numericCol) != -1)
                    tr += "<input type='text' style='width: 90%;text-align:center; border-style: none;background-color:#fff;text-align:right;' value='"+value+"' onkeypress='return isNumericWithOneDecimal(event)' onmousedown='whichButton(event)' onfocus='Focus(this,0)' onkeydown='return noCTRL(event)' onblur=\"CalculateTotal(this," + tableName + "," + j + ",this.value)\"/>";
                else if ($.inArray("" + j + "", dateCol) != -1){
                    if (value == "")
                        value = d.localeFormat("dd-MMM-yyyy");
                    tr += "<input type='text' style='width: 70%; border-style: none;background-color:#fff' autocomplete='off' value='" + value + "' class='dtp'/>";
                }
                else if ($.inArray("" + j + "", fileUploadCol) != -1 && isfileUpload)
                    tr += "<input type='file' size='20' style='width:71%;'/>";
                else if ($.inArray("" + j + "", IfsccolIndex) != -1)
                    tr += "<input type='text' flg='banklist' style='width: 93%; border-style: none;background-color:#fff' class='clsbranch' value='" + value + "' />";
                else if ($.inArray("" + j + "", bankcolIndex) != -1)
                    tr += "<input type='text' flg='banklist' style='width: 93%; border-style: none;background-color:#fff' class='clsbranch' bankid='" + value + "' value='" + bankName + "' />";
        ...
        if (tableName == "tbldd" && result != "") {
            ...
            for (var i = 0; i < tr.length; i++) {
                $($(tr)[i].cells[4].children[0]).attr("bankid",result[i].BankId);
            }
            ...
            for (var i = 0; i < tr.length; i++) {
                ...
                $($(tr)[i].cells[5].children[0]).attr("bankid",result[i].BankId);
            }
...
function fnAutomaticAdjustMent() {
  //  debugger;
    ...
    if (rows.length > 0) {
        ...
        //}
        var totalamount = parseFloat($("#lblTotalInvoiceAmount").html());
       var balance = sumOfTotalAmount - totalamount;
        for (var i = 0; i < rows.length; i++) {
            var amount = $(rows)[i].cells[5].innerHTML;
            var CashDiscamount = $(rows)[i].cells[7].children[0].value;
            amount = parseFloat(amount)-parseInt(CashDiscamount);
            if (balance > amount) {
                balance = balance - amount;
                $(rows)[i].cells[6].children[0].value = parseFloat(amount).toFixed(2);
                totalamount = parseFloat(totalamount) + parseFloat(amount);
                $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", amount);
            }
            else {
                var amt = $(rows)[i].cells[6].children[0].value;
                if (parseFloat(amt) + parseFloat(balance) > parseFloat(amount)) {
                    $(rows)[i].cells[6].children[0].value = parseFloat(amount).toFixed(2);
                    balance = (parseFloat(amt) + parseFloat(balance)) - parseFloat(amt);
                    $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", amount);
                } else {
                    $(rows)[i].cells[6].children[0].value = parseFloat(parseFloat(amt) + parseFloat(balance)).toFixed(2);
                    totalamount = parseFloat(totalamount) + parseFloat(balance);
                    $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", parseFloat(amt) + parseFloat(balance));
                    balance = 0;
                    break;
                }
            }   
...
// Start here Automatic Adjustment 
function fnAutomaticAdjustMentFIFO(flg) {
    //  debugger;
    var sumOfTotalAmount = 0;
    var amount1 = parseFloat($("#spnTotalCash").html());
    var amount2 = parseFloat($("#spnTotalDD").html());
    ...
    if (rows.length > 0) {
        ...
        for (var i = 0; i < totLength; i++) {
            ...
            var amount = $(rows)[cnt].cells[5].innerHTML;
            var CashDiscamount = $(rows)[cnt].cells[7].children[0].value;
            amount = parseFloat(amount) - parseInt(CashDiscamount);
            if (sumOfTotalAmount > amount) {
                sumOfTotalAmount = sumOfTotalAmount - amount;
                $(rows)[cnt].cells[6].children[0].value = parseFloat(amount).toFixed(2);
                $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", amount);
            }
            else if (sumOfTotalAmount > 0) {
                $(rows)[cnt].cells[6].children[0].value = parseFloat(sumOfTotalAmount).toFixed(2);
                $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", sumOfTotalAmount);
                sumOfTotalAmount = 0;
            }
            else {
                $($(rows)[cnt].cells[6].children[0]).attr("addjustedamt", 0);
                $(rows)[cnt].cells[6].children[0].value = 0;
            }

            totalamount = parseFloat(totalamount) + parseFloat($(rows)[i].cells[6].children[0].value);
        }
        $("#lblTotalInvoiceAmount").html(parseFloat(totalamount).toFixed(2));
        var balance = parseFloat(parseFloat($("#lblTotalInvoiceAmount").html()) - parseFloat($("#lblTotalPaidAmount").html())).toFixed(2);
        if (balance < 0)
